...
module.exports = class productRegistedModel {
  ...
  static async syncProduct(dataUpdate) {
    let logBase = `models/productRegistedModel.syncProduct `;
  ...
  static async selectProductsRegistered(
    ...
    try {
      ...
      if (status && status !== "O") {
        ...
        whereSub += ` AND PM.P_STATUS = '${status}'  `;
      ...
      if (product_only_brgn && product_only_brgn === 1) {
        ...
        whereSub +=
          " AND PM.P_SALE_PRICE > 0 AND  (PP.PS_STATUS NOT IN ('A') OR PP.PS_STATUS IS NULL)  ";
      }
      ...
      FROM (SELECT SUB.*,
              (SELECT MCL.CTGRY_NAME FROM ${
                dataConnect.M_DB_CONNECT
              }.MART_CTGRY MCL WHERE SUB.CTGRY_LARGE_NO = MCL.CTGRY_LARGE_NO AND SUB.M_POS_REGCODE = MCL.MART_SEQNO AND MCL.CTGRY_MEDIUM_NO = 0 AND MCL.CTGRY_SMALL_NO = 0 AND MCL.CTGRY_STATE = 1 LIMIT 1) AS P_CAT,
              (SELECT MCL.CTGRY_NAME FROM ${
                dataConnect.M_DB_CONNECT
              }.MART_CTGRY MCL WHERE SUB.CTGRY_LARGE_NO = MCL.CTGRY_LARGE_NO AND SUB.M_POS_REGCODE = MCL.MART_SEQNO AND MCL.CTGRY_MEDIUM_NO = SUB.CTGRY_MEDIUM_NO AND MCL.CTGRY_SMALL_NO = 0 AND MCL.CTGRY_STATE = 1 LIMIT 1) AS P_CAT_MID,
              CASE
              ...
              FROM (SELECT
                  ...
                  (SELECT CONCAT(IF(PUNIT.GOODS_STD IS NOT NULL, CONCAT(' ', PUNIT.GOODS_STD), ''), IF(PUNIT.EXTNS_UNIT_COUNT IS NOT NULL
                      ...
                      FROM ${
                        dataConnect.M_DB_CONNECT
                      }.MART_ORDER_GOODS as PUNIT WHERE PUNIT.USE_YN = 'Y' 
                      AND PUNIT.MART_SEQNO = MOG.MART_SEQNO AND PUNIT.GOODS_CODE = MOG.GOODS_CODE 
                      AND PUNIT.BRCD = MOG.BRCD order by PUNIT.MART_ORDER_GOODS_SEQNO desc limit 1) AS P_UNIT,
                  PM.P_STATUS AS P_STATUS,
                  PM.C_TIME AS C_TIME,
                  PM.M_TIME AS M_TIME,
                  (SELECT EXPSR_PRICE FROM ${
                    dataConnect.M_DB_CONNECT
                  ...
                  (SELECT CTGRY_LARGE_NO FROM ${
                    dataConnect.M_DB_CONNECT
                  }.MART_CTGRY WHERE CTGRY_SEQNO = MOG.CTGRY_SEQNO AND MART_SEQNO = MOG.MART_SEQNO LIMIT 1) AS CTGRY_LARGE_NO,
                  (SELECT CTGRY_MEDIUM_NO FROM ${
                    dataConnect.M_DB_CONNECT
                  }.MART_CTGRY WHERE CTGRY_SEQNO = MOG.CTGRY_SEQNO AND MART_SEQNO = MOG.MART_SEQNO LIMIT 1) AS CTGRY_MEDIUM_NO,
                  (SELECT MC.CTGRY_NAME FROM ${
                    dataConnect.M_DB_CONNECT
                  }.MART_CTGRY AS MC WHERE MC.MART_SEQNO = MOG.MART_SEQNO AND MC.CTGRY_SEQNO = MOG.CTGRY_SEQNO LIMIT 1) AS P_CAT_SUB,
                  (SELECT MC.CTGRY_SEQNO FROM ${
                    dataConnect.M_DB_CONNECT
                  }.MART_CTGRY AS MC WHERE MC.MART_SEQNO = MOG.MART_SEQNO AND MC.CTGRY_SEQNO = MOG.CTGRY_SEQNO LIMIT 1) AS SEQ_P_CAT_SUB,
                  (SELECT P.PRVDR_NAME FROM ${
                    dataConnect.M_DB_CONNECT
                  }.PRVDR AS P WHERE P.MART_SEQNO = MOG.MART_SEQNO AND P.PRVDR_SEQNO = MOG.PRVDR_SEQNO 
                  AND P.PRVDR_STATE_CODE = 'Y' LIMIT 1) AS P_PRVDR_NAME,
                  PM.P_MIN_STOCK,
  ...
  static async selectProductByArraySeq(prd_seqs) {
    let logBase = `models/productRegistedModel.selectProductByArraySeq:`;
    try {
  ...
  static async searchProductImages(
    ...
    let logBase = `models/productRegistedModel.searchProductImages:`;
    try {
      let where = ` WHERE 1=1 `;
  ...
  static async getArryCateOfImage(dbConnect, pos_regcode) {
    let logBase = `models/productRegistedModel.getArryCateOfImage:`;
    try {

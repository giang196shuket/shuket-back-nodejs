...
module.exports = {
  async moaSearchList(req, res, next) {
    ...
    data.forEach((row) => {
      ...
      list.push({
        ...
        is_tdc: statusStock,
        time_sync_tdc : (row.IS_STOCK == 'Y' || row.IS_STOCK == 'F') ? moment(row.INITIAL_STOCK_DATE).format('YYYY-MM-DD hh:mm:ss') : 'NA',
        time_last_sync_tdc : (row.IS_STOCK == 'Y' && row.LAST_STOCK_DATE != "") ?  moment(row.LAST_STOCK_DATE).format('YYYY-MM-DD hh:mm:ss') : 'NA',
        text_color : ((i + 1) % 2 == 0) ? 'background-color: #504f4f69;' : 'background-color: #807e7e4f;',
  ...
  async getDetailMart (req, res, next) {
    ...
    const getListPayment = await martModel.getListPaymentOfMart(row.M_MOA_CODE)
    ...
    getListPayment.forEach(item => {
      ...
      dataPaymentReturn.push({
        ...
        is_insert: (item.IS_INSERT === 'Y' || item.IS_INSERT === 'N') ? 'NO' : 'YES'
      })
    ...
    if(arrayAnotherApp.includes(row.M_TYPE) && (row.M_MOA_CODE  == row.M_HQ_CODE)){
      type = 'H' //main mart
    }
    if(arrayAnotherApp.includes(row.M_TYPE) && (row.M_MOA_CODE  != row.M_HQ_CODE)){
      type = 'F'
    ...
    if(row.M_APP_TYPE != 'SG' && row.M_TYPE === 'F' && row.M_MOA_CODE !== row.M_HQ_CODE){
      showfranchise = 'Y';
    } else {
      showfranchise = 'N';
    }
    ...
    if(row.M_S_DATE_SERVICE == '0000-00-00 00:00:00' || row.M_S_DATE_SERVICE == null){
      sDateService = ""
    }else{
      sDateService = moment(row.M_S_DATE_SERVICE).format('YYYY-MM-DD')
    }

    if(row.M_S_DATE_BILLING == '0000-00-00 00:00:00' || row.M_S_DATE_SERVICE == null){
      sDateBilling = ""
    }else{
    ...
    let martInfo = {
      ...
      logo_push_url : row.M_LOGO ? await loadImageAws(row.M_LOGO_PUSH, 'mart/logo') : "",
      ...
      bizhour_close: bizhourArr.substring(bizhourArr.indexOf(":")+1,bizhourArr.length),
      ...
      hidecheckbox:  (row.IS_STOCK === 'Y' && row.INITIAL_STOCK_DATE !== "") ? 1 :0,
    ...
    const configCustom = await martModel.getDataConfigCustomMart(row.M_MOA_CODE)
    let receiveOption = 'N'
    ...
    const dataResponse = {
      mart_info: {...martInfo, receive_option : receiveOption , receive_begin_hours: receiveBeginHours, receive_end_hours: receiveEndHours},

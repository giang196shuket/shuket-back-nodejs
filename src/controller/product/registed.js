...
const { messageError, messageSuccess, typeLog } = require("../../helper/message");
...
const { generateTag, arrayColumn, arrayColumnAssign } = require("../../helper/funtion");
const { loadImageAws, loadNoImage, loadImageAwsProduct } = require("../../service/loadImage");
const ip = require('ip');
...
module.exports = {
  async searchProductRegisteredList(req, res, next) {
    ...
    const dataConnect = await queriesHelper.getDBconnect(user.u_martid)
    ...
    if(willBeRun == 1){
      ...
      await queriesHelper.updateTableWhere('TBL_MOA_USERS_ADMIN', `U_LSYNCPRD = '${moment().format('YYYY-MM-DD HH:mm:ss')}'`,`
       U_ID = '${user.user_id}' AND U_MARTID = '${user.u_martid}'`)
       const productsMainPos = await productRegistedModel.getProductRegister_Main_Pos(dataConnect.M_DB_CONNECT, user.u_martid)
       let dataUpdate = []
    ...
    let defaultStock = 5 // sẽ cảnh báo khi số lượng product bé hơn hoặc bằng 5
     checkUseStock = await queriesHelper.getRowDataWhere('TBL_MOA_MART_CONFIG', `M_MOA_CODE = '${user.u_martid}'`)
    let initialStock = 0
    let isUsingStock = 0
    ...
    let {keyword_type, keyword_value, category_code, category_sub_code, date_start, date_end, page, limit, order_by, prd_status, option_check_stock, stock_search_value, sort_prd_stock, product_no_image,  product_only_brgn, is_delivery } = req.body
    page = !page ? 1 : page
    limit = !limit ? 1 : limit
    const offset = page * limit - limit
    // lấy list product hiện tại sau khi sync xong
    const dataListProduct = await productRegistedModel.selectProductsRegistered(checkUseStock, dataConnect, prd_status, option_check_stock, stock_search_value, keyword_type, keyword_value, category_code , category_sub_code, date_start, date_end, product_no_image, product_only_brgn, sort_prd_stock, offset, limit)
    ...
    if(listPCode.length > 0 && initialStock == 1 ){
      //trả ra product nào đang dùng stock
       const dataProductUseStock = await queriesHelper.getListDataFieldWhere('GOODS_CODE AS P_CODE, STK_STOCK, C_TIME, M_TIME', 'TBL_MOA_STOCK_TOGETHERS',
        ` M_MOA_CODE = '${user.u_martid}' AND GOODS_CODE IN (${listPCode})` )
       if(dataProductUseStock.length > 0){
         isMerge = 1
         dataMergeStock = arrayColumnAssign(dataProductUseStock,  'STK_STOCK', 'P_CODE')
         dataMergeStockCtime = arrayColumnAssign(dataProductUseStock,  'C_TIME', 'P_CODE')
         dataMergeStockMtime = arrayColumnAssign(dataProductUseStock,  'M_TIME', 'P_CODE')
       }
    }
    ...
    for (const row of dataListProduct.rows) {
          ...
          if(row.PRICE_TYPE === 'PC'){
            ...
            if(row.PRICE_UP_DOWN === 'U'){
              ...
              if(row.P_SALE_PRICE && row.P_SALE_PRICE > 0){
                ...
                priceShow = ( parseInt(row.P_SALE_PRICE) / 100) *  parseInt(row.PRICE_NUMBER) + parseInt(row.P_SALE_PRICE)
              }else{
                //P_LIST_PRICE : price mặc định
                priceShow = ( parseInt(row.P_LIST_PRICE) / 100) *  parseInt(row.PRICE_NUMBER) +  parseInt(row.P_LIST_PRICE)
              }
            }
            if(row.PRICE_UP_DOWN === 'D'){
              ...
              if(row.P_SALE_PRICE && row.P_SALE_PRICE > 0){
                //P_SALE_PRICE: price đang sale
                // PRICE_NUMBER : giá trị mở rộng
                priceShow =  parseInt(row.P_SALE_PRICE) - ( parseInt(row.P_SALE_PRICE) / 100 ) *  parseInt(row.PRICE_NUMBER)
              }else{
                //P_LIST_PRICE : price mặc định
                priceShow =  parseInt(row.P_LIST_PRICE) - ( parseInt(row.P_LIST_PRICE) / 100 ) *  parseInt(row.PRICE_NUMBER)
              }
            }
          }
          if(row.PRICE_TYPE === 'AM'){
            ...
            if(row.PRICE_UP_DOWN === 'U'){
              ...
              if(row.P_SALE_PRICE && row.P_SALE_PRICE > 0){
                //P_SALE_PRICE: price đang sale
                // PRICE_NUMBER : giá trị mở rộng
                priceShow =  parseInt(row.P_SALE_PRICE) +  parseInt(row.PRICE_NUMBER)
              }else{
                //P_LIST_PRICE : price mặc định
                priceShow =  parseInt(row.P_LIST_PRICE) +  parseInt(row.PRICE_NUMBER)
              }
            }
            if(row.PRICE_UP_DOWN === 'D'){
              // DOWN PRICE
      ...
      list[i] = {
        ...
        code: row.P_CODE,
        name: row.P_NAME,
        category: row.P_CAT + " " + (row.P_CAT_MID ? ` > ${row.P_CAT_MID}` : " ") + " " + (row.P_CAT_SUB ? ` > ${row.P_CAT_SUB}` : " "),
        unit :row.P_UNIT,
        ...
        time_start: row.TIME_START ? moment(row.TIME_START).format('YYYY-MM-DD') : null,
      ...
      productImages.forEach(prdImage => {
        arrImage[j]  = loadImageAwsProduct(prdImage, 'product/cover')
          if(prdImage.main === 1){
      ...
      if(dataMergeStock[row.P_CODE] && isMerge === 1){
        ...
        }else if(moment(dataMergeStockMtime[row.P_CODE]).isSameOrAfter(moment(dataMergeStockCtime[row.P_CODE]))){
           // thời gian sync stock
    ...
    const checkUseMaxQty = await queriesHelper.getRowDataWhere('TBL_MOA_MART_CONFIG', ` M_MOA_CODE = '${user.u_martid}'`)
    let dataResponse = {
      page_index: page,
      page_size: limit,
      page_count: Math.ceil( dataListProduct.search_count / limit),
      search_count : dataListProduct.search_count, 
      list_product: list,
    ...
    return res
      .status(200)
      .json(
        responseSuccess(200, messageSuccess.Success, dataResponse)
      );
  },
  ...
  async updateStockItem(req, res, next) {
    ...
    await queriesHelper.updateTableWhere('TBL_MOA_PRD_MAIN', `P_MIN_STOCK = ${min_stock}, M_ID = '${user.user_id}', M_TIME = '${time}' ` , `
  ...
  async setMaxMinProduct(req, res, next) {
    let {seq, is_pro_maxqty, is_pro_minqty, pro_max_qty, pro_min_qty} = req.body
    const user = req.userInfo
    const time = moment().format('YYYY-MM-DD HH:mm:ss')
    ...
    const resultUpdate = await queriesHelper.updateTableWhere('TBL_MOA_PRD_MAIN', `
    ...
    if(resultUpdate > 0){
      ...
      `'${user.u_martid}', '${ip.address()}', '${typeLog.SetCustomMaxMinQty}', '${dataRequest}', '${time}','${user.user_id}'` )
    }
    return res
    .status(200)
    .json(responseSuccess(200, messageSuccess.Success, messageSuccess.Success));
  ...
  async productStockStatus(req, res, next) {
    ...
    for (const row of rows) {
      ...
      const resultUpdate = await queriesHelper.updateTableWhere('TBL_MOA_PRD_MAIN',` P_INV_TYPE = ${prd_stock_status} , 
      M_ID = '${user.user_id}', M_TIME = '${time}'`, ` SEQ = '${row.SEQ}'`)
      if(resultUpdate > 0){
  ...
  async viewDetailProduct(req, res, next) {
    ...
    const dataConnect = await queriesHelper.getDBconnect(req.userInfo.u_martid)
    // Get cate for search by images
    const arrCate = await productRegistedModel.getArryCateOfImage(dataConnect.M_DB_CONNECT, dataConnect.M_POS_REGCODE)
  ...
  async searchProductImages(req, res, next) {
    ...
    const dataConnect = await queriesHelper.getDBconnect(user.u_martid)
    // Get images of mart
    const dataImages = await productRegistedModel.searchProductImages(keyword, img_barcode, img_type, img_keyword, img_cate,dataConnect.M_DB_CONNECT, dataConnect.M_POS_REGCODE)
    let listImages = []

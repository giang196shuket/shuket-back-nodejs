...
// arrMerge ko chứ các dept1 là cha của các thằng dept2 => kiếm bằng query
  ...
  for await (const cateUser of arrMerge) {
    ...
    if (cateUser.U_CATE_DEPT === 3) {
      const data = await userModel.selectProgByCode(cateUser.U_CATE_PCD); // thằng dept 2 của dept 3 hiện tại
      const parentIndex = list.findIndex((ls) => ls.group_code === data.U_CATE_PCD) // kiếm thằng cha  dept 1 của dept 2
      if(list[parentIndex]){
